...
import numpy as np
from scipy.sparse.linalg import eigsh, spsolve, LinearOperator, ArpackNoConvergence, eigs, lobpcg, inv, splu, norm
...
def mixed_infsup(matB, matH, matL):
    ...
    try:
        ...
        mineig, _ = eigsh(A = operator, k = m-1, M = matL, which = 'SA', tol = 1e-5)

        print("Eigenvalues = ", mineig, flush=True)
        print("shape of eigenvalues = ", mineig.shape, flush=True)
        print("m = ", m, flush=True)
        mineigenValue = mineig[m-rank]
        maxeigenValue = mineig[-1]
...
def mixed_infsup_C(matB, matH, matC):
    ...
    Cinv = np.linalg.inv(Cdense)
    BtCinvB = Bdense.T @ Cinv @ Bdense

    # We will evaluate the rank of the matrix B*H^(-1)*B^T
    rank = np.linalg.matrix_rank(BtCinvB)
    n,n = BtCinvB.shape

    rankB = np.linalg.matrix_rank(Bdense)

    print("Dimension of the square matrix B^T*C^(-1)*B = ", n, flush=True)
    print("Rank of  B^T*C^(-1)*B = ", rank, flush=True)
    print("Dimension of the matrix B = ", Bdense.shape, flush=True)
    print("Rank of  B = ", rankB, flush=True)
    ...
    try:
        # Calculamos el menor autovalor
        mineig, _ = eigsh(A = operator, k = n-1, M = matH, which = 'SA', tol = 1e-5)

        print("Eigenvalues = ", mineig, flush=True)
        print("shape of eigenvalues = ", mineig.shape, flush=True)
        print("n = ", n, flush=True)
        mineigenValue = mineig[n-rank]
        maxeigenValue = mineig[-1]
        eigenValues = [mineigenValue, maxeigenValue]
        return eigenValues 
...
def primal_infsup(matM, matH, eps):
    """
